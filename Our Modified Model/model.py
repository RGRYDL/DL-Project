...
class cyclegan(object):
    ...
    def _build_model(self):
        ...
        self.batch_images = tf.placeholder(tf.float32,
                                        [10, self.image_size, self.image_size,
                                         self.input_c_dim + 1 + self.output_c_dim],
                                        name='real_A_B_C_images')
        self.real_data = tf.placeholder(tf.float32,
                                        [1, self.image_size, self.image_size,
                                         self.input_c_dim + 1 + self.output_c_dim],
                                        name='real_A_B_C_image')

        self.batch_imagesA = self.batch_images[:, :, :, :self.input_c_dim]
        ...
        #self.fake_BB = self.fake_B
        #self.fake_B = tf.where(tf.greater(self.fake_B, 0), self.fake_B, tf.zeros_like(self.fake_B)) #二值化
        #self.fake_B = tf.where(tf.less_equal(self.fake_B, 0), self.fake_B, tf.ones_like(self.fake_B))
        ...
        self.lambda4 = tf.placeholder(tf.float32,name='lambda4')

        self.g_lossx = 0.0
        for idx in range(10):
            self.img = tf.gather(self.batch_imagesA, [idx])
            self.fake_imgB = self.generator(self.img, self.options, 1, True, name="generatorA2B")
            self.fake_imgC = self.generator(tf.concat([self.img,self.fake_imgB], 3), self.options, self.options.output_c_dim, True, name="generatorAB2C")
            self.g_lossx += abs_criterion(self.fake_C, self.fake_imgC)

        self.g_lossx *= self.lambda4
        ...
        #self.testB = tf.where(tf.greater(self.testB, 0), self.testB, tf.zeros_like(self.testB)) #二值化
        #self.testB = tf.where(tf.less_equal(self.testB, 0), self.testB, tf.ones_like(self.testB))
    ...
    def train(self, args):
        ...
        for epoch in range(args.epoch):
            ...
            epoch_count += 1.0
            lambda4 = (epoch_count / 200.0) * 5.0

            batch_idxs = min(min(len(dataA), len(dataB), len(dataC)), args.train_size) // 10 #78
            lr = args.lr if epoch < args.epoch_step else args.lr*(args.epoch-epoch)/(args.epoch-args.epoch_step)

            for idx in range(0, batch_idxs): #0 - 78
                batch_files = list(zip(dataA[idx * 10: (idx + 1) * 10],
                                       dataB[idx * 10: (idx + 1) * 10],
                                       dataC[idx * 10: (idx + 1) * 10]))
                #print(batch_files)
                batch_images = [load_train_data(batch_file, args.load_size, args.fine_size) for batch_file in batch_files]
                batch_images = np.array(batch_images).astype(np.float32)

                h1 = int(np.ceil(np.random.uniform(1e-2, args.load_size-args.fine_size)))
                w1 = int(np.ceil(np.random.uniform(1e-2, args.load_size-args.fine_size)))
                r1 = np.random.random()

                batch_images_new = np.empty([10,256,256,7])

                #print("a",batch_images.shape)

                for idxx in range(10): #1-10
                    img = batch_images[idxx]
                    #print("b",img.shape)
                    img = img [h1:h1+args.fine_size, w1:w1+args.fine_size]
                    #print("c",img.shape)
                    if r1 > 0.5: img = np.fliplr(img)
                    batch_images_new[idxx] = img

                #print("d",batch_images_new.shape)

                #c = 0
                for img in batch_images_new: #1-10
                    img = img[np.newaxis, :, :, :]
                    #c+=1

                    #AtoSave = img[:,:,:,:3]
                    #save_images(AtoSave, [1, 1], 'C:/Users/Kioku/Desktop/s/{:02d}_{:02d}_A.jpg'.format(idx, c))
                    #BtoSave = img[:,:,:,3:4]
                    #save_images(BtoSave, [1, 1], 'C:/Users/Kioku/Desktop/s/{:02d}_{:02d}_B.jpg'.format(idx, c))
                    #CtoSave = img[:,:,:,4:7]
                    #save_images(CtoSave, [1, 1], 'C:/Users/Kioku/Desktop/s/{:02d}_{:02d}_C.jpg'.format(idx, c))
                    ...
                    g_lossx, g_loss, _, summary_str = self.sess.run(
                        [self.g_lossx, self.g_loss, self.g_optim, self.g_sum],
                        feed_dict={self.real_data: img, self.lr: lr, self.batch_images: batch_images_new, self.lambda4: lambda4})
                    self.writer.add_summary(summary_str, counter)
                        
                    #[fake_B, fake_C] = self.pool([fake_B, fake_C])

                    counter += 1
                    print(("Epoch: [%2d] [%4d/%4d] time: %4.4f d_loss: %4.4f g_loss: %4.4f g_lossx: %4.4f" % (
                        epoch, idx, batch_idxs, time.time() - start_time, d_loss, g_loss, g_lossx)))
